...
public static void main(String[] args) {}
```
Метод **Main()** является отправной точкой выполнения для всех Java-приложений.
**String args[]** — это массив строковых объектов, которые нам нужно передать из аргументов командной строки.
Каждое Java-приложение должно иметь по крайней мере один основной метод.

#### Что такое конструктор в Java?
**Конструктор** — это специальный метод, используемый для инициализации объектов в Java.
Мы используем конструкторы для инициализации всех переменных в классе при создании объекта. Когда объект создается он инициализируется автоматически с помощью конструктора в Java.
...
Str.length(); // вернётся число 11.
```
**length:** -> возвращает количество значений или объектов в массиве.
```
String days[]={” Sun”,”Mon”,”wed”,”thu”,”fri”,”sat”}; // Вернет 6, так как количество значений в массиве days равно 6.
```
